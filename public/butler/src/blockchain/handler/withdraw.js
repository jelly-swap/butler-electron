                    ...
                    if (!isProcessed && !this.localCache[withdraw.id]) {
                        try {
                            ...
                            try {
                                ...
                                logger_1.logError(`WITHDRAW_SERVICE_ERROR: ${err}`);
                            }
                        }
                        catch (err) {
                        ...
                        logger_1.logWarn('WITHDRAW_ALREADY_PROCESSED', withdraw.id);
                    }
                }
            }
            else {
                logger_1.logWarn('WITHDRAW_SWAP_NOT_FOUND', withdraw.id);
            }
        });
    }
    processOldWithdraws() {
